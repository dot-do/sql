...
- [Client SDK Migration](#client-sdk-migration)
  - [From D1 Client to sql.do Client](#from-d1-client-to-sqldo-client)
...
Both D1 and DoSQL use SQLite-compatible SQL, but there are some differences:
...
SELECT *, vector_distance(embedding, ?) as distance FROM documents ORDER BY distance;
...
async function importToDoSQL(dosql: Database, data: Map<string, unknown[]>): Promise<void> {
...
// D1
try {
  await env.DB.prepare('INSERT INTO users (email) VALUES (?)').bind('duplicate@test.com').run();
} catch (error) {
...
try {
  await db.run('INSERT INTO users (email) VALUES (?)', ['duplicate@test.com']);
} catch (error) {
...
### From D1 Client to sql.do Client
...
class D1CompatibleStatement {
  private params: unknown[] = [];

  constructor(private stmt: PreparedStatement) {}
  ...
  async all<T>(): Promise<D1Result<T>> {
    const results = await this.stmt.all<T>(this.params);
    return {
      results,
      success: true,
  ...
  async first<T>(): Promise<T | null> {
    return this.stmt.get<T>(this.params);
  }

  async run(): Promise<D1Result<unknown>> {
    const result = await this.stmt.run(this.params);
    return {
      results: [],
      success: true,
      meta: {
...
const result = await d1Db.select().from(users).where(eq(users.active, true));
...
const result = await dosqlDb.select().from(users).where(eq(users.active, true));
```
...
class MigrationTestHarness {
  ...
  async compareQuery<T>(
    ...
    const d1Result = await this.d1.prepare(sql).bind(...(params || [])).all<T>();
  ...
  async runComparisonSuite(): Promise<ComparisonReport> {
    ...
    const queries = [
      ...
      { sql: 'SELECT u.*, COUNT(p.id) as post_count FROM users u LEFT JOIN posts p ON p.user_id = u.id GROUP BY u.id', params: [] },
    ];
...
async function verifyDataConsistency(
  ...
  for (const { name: table } of d1Tables.results) {
    // Compare row counts
    const d1Count = await d1.prepare(`SELECT COUNT(*) as count FROM ${table}`).first<{ count: number }>();
...
async function computeTableChecksum(d1: D1Database, table: string): Promise<string> {
  const rows = await d1.prepare(`SELECT * FROM ${table} ORDER BY rowid`).all();
  return hashRows(rows.results);
}

async function computeTableChecksumDoSQL(dosql: Database, table: string): Promise<string> {
...
async function runPerformanceBenchmark(
  ...
  results.push(await benchmarkOperation(
    'INSERT',
    async () => {
      await d1.prepare('INSERT INTO benchmark_test (value) VALUES (?)').bind(Math.random()).run();
    },
    async () => {
...
function computeStats(times: number[]): { avgMs: number; p95Ms: number; p99Ms: number } {
...
async function verifyRollback(d1: D1Database, dosql: Database): Promise<boolean> {
  const tables = ['users', 'posts', 'comments'];

  for (const table of tables) {
    const d1Count = await d1.prepare(`SELECT COUNT(*) as c FROM ${table}`).first<{ c: number }>();
    const dosqlCount = await dosql.queryOne<{ c: number }>(`SELECT COUNT(*) as c FROM ${table}`);

    if (d1Count?.c !== dosqlCount?.c) {
      console.error(`Count mismatch in ${table}: D1=${d1Count?.c}, DoSQL=${dosqlCount?.c}`);
      return false;
    }
  }
...
- Full rollback: Re-migrate all data from DoSQL to D1
...
async function canRollback(state: MigrationState): Promise<boolean> {
  ...
  const hoursBehind = (Number(state.dosqlLastLSN - state.d1LastLSN)) / 3600;
