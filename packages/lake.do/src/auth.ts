/**
 * lake.do/auth - Integration with oauth.do for seamless authentication
 *
 ...
 * @example
 * ```typescript
 * import { createAuthenticatedClient } from 'lake.do/auth';
 *
...
import type { LakeClient } from './types.js';

/**
 * Options for creating an authenticated Lake client.
 *
 * @public
 * @since 0.2.0
 */
export interface AuthenticatedLakeClientOptions {
  /** DoLake endpoint URL */
  url: string;
...
/**
 * Creates a Lake client authenticated via oauth.do.
 *
 * @description Obtains an auth token from oauth.do (reusing an existing session
 * if available, or initiating a new auth flow) and creates a Lake client with it.
 *
 * Requires `oauth.do` to be installed as a peer dependency.
 *
 * @param options - Client and auth configuration
 * @returns A fully authenticated LakeClient instance
 ...
 * const client = await createAuthenticatedClient({
 *   url: 'https://lake.example.com',
 *   authOptions: { scopes: ['lake:read', 'lake:write'] },
 * });
 *
 * const result = await client.query('SELECT * FROM orders');
 * await client.close();
 ...
 * @since 0.2.0
 */
export async function createAuthenticatedClient(options: AuthenticatedLakeClientOptions): Promise<LakeClient> {
  ...
  return createLakeClient({ ...options, token });
}
 ...
 * @example
 * ```typescript
 * import { ensureAuth } from 'lake.do/auth';
 *
 ...
 * @example
 * ```typescript
 * import { getAuthToken } from 'lake.do/auth';
 *
